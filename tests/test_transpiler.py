...
class TestTranspilerOnGates(TestCase):
    def _generate_params(self, name, varnames):
        params = {}
        rot_args = ['theta', 'phi', 'lam', 'gamma', 'v_x', 'v_y', 'v_z', 'scaling', 'f_x', 'slope', 'offset']
        
        if name == 'EvolvedOperatorAnsatz':
            operators = (Z ^ Z) - 0.1 * (X ^ I)
            return {'operators': operators, 'parameter_prefix': 'x'}
        
        if name == 'QAOAAnsatz':
            operators = (Z ^ Z) - 0.1 * (X ^ I)
            return {'cost_operator': operators, 'parameter_prefix': 'x'}
        
        if name == 'IntegerComparator':
            return {'num_state_qubits': 3, 'value': 7}
        ...
        if name in ['GraphState', 'HiddenLinearFunction']:
            return {'adjacency_matrix': [[1, 0, 0], [0, 1, 1], [0, 1, 1]]}
        
        if name == 'IQP':
            return {'interactions': [[6, 5, 3], [5, 4, 5], [3, 5, 1]]}

        if name == 'PhaseEstimation':
            qc = QuantumCircuit(1)
            qc.h(0)
            return {'num_evaluation_qubits': 1, 'unitary': qc}
        ...
        if name == 'PiecewiseChebyshev':
            return {'f_x': lambda x: np.arcsin(1 / x),  'degree': 2, 'breakpoints': [2, 4], 'num_state_qubits': 2}
        
        if name in ['MCMT', 'MCMTVChain']:
            return {'gate': HGate(), 'num_ctrl_qubits': 2, 'num_target_qubits': 2}
        
        if name == 'LinearFunction':
            return {'linear': [[1, 0, 0], [1, 1, 0], [0, 0, 1]]}
        ...
        for ra in rot_args:
            if name in ['ExcitationPreserving', 'DraperQFTAdder', 'RGQFTMultiplier', 'QuadraticForm']:
                break
            if ra in varnames:
        ...
        for nqbis in ['num_ctrl_qubits', 'num_qubits', 'num_state_qubits', 'num_variable_qubits', 'feature_dimension', 'num_evaluation_qubits']:
    ...
    def _get_qiskit_gates(self):
        qiskit_gates = {attr: None for attr in dir(qiskit.circuit.library) if attr[0] in string.ascii_uppercase}

        for gate_name in qiskit_gates:
            params = self._generate_params(gate_name, getattr(qiskit.circuit.library, gate_name).__init__.__code__.co_varnames)
            try:
                qiskit_gates[gate_name] = getattr(qiskit.circuit.library, gate_name)(**params)
            except:
                continue
    ...
    def test_qiskit_gates_transpilation(self):
        ...
        for gate_name, gate in qiskit_gates.items():
            with self.subTest(gate=gate):
                ...
                if gate_name in ['Barrier', 'IGate', 'Measure', 'NLocal', 'QuadraticForm', 'Reset', 'TwoLocal', 'XOR', 'GMS', 'Permutation']:
                    source_circuit.h(0)
